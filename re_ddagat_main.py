...
from re_ddagat_model import DDAGAT
...
def evaluate(args, model, tokenizer, processor, device, mode="test", output_dir='./'):
    ...
    if args.task_name == 'semeval':
        ...
        rel_size = len(label_map) - 1
        ignore_label_id = label_map.get('Other', -1)
        result = measure_statistics(preds, out_label_ids, rel_size, ignore_label_id)
...
def train_func(args):
    ...
    model = DDAGAT.from_pretrained(args.model_path, config=config)
