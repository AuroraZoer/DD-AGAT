...
def compute_micro_f1(preds, labels, label_map, ignore_label, output_dir=None):
    ...
    if 'micro avg' in res:
        ...
        rel_size = len(label_map) - (1 if ignore_label in label_map else 0)
...
def measure_statistics(preds, labels, rel_size, ignore_label):
    ...
    mask_t = torch.eq(t, ignore_label)
    mask_p = torch.eq(y, ignore_label)

    true = torch.where(mask_t, label_num, t)
    pred = torch.where(mask_p, label_num, y)
